...
(function() {
  ...
  let redisConnectionHealthy = true;
  ...
  async function sendWebhook(message) {
    ...
    try {
      ...
      // Use global.fetch if available (for testing), otherwise original.fetch
      const fetchFn = (typeof global !== 'undefined' && global.fetch) || original.fetch;
      if (fetchFn) {
        await fetchFn(config.webhook, {
          method: 'POST',
  ...
  async function init(options = {}) {
    ...
    return {
      getCost: () => {
      // Return null only when Redis is enabled AND healthy
      if (redisClient && redisConnectionHealthy) return null;
      // Otherwise return local cost (for no Redis or Redis failed)
      return totalCost;
    },
